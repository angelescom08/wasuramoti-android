...
class KarutaPlayer(var activity:WasuramotiActivity,val reader:Reader,val cur_num:Int,val next_num:Int) extends BugReportable{
  ...
  def play(bundle:Bundle,fromAuto:Boolean=false,fromSwipe:Boolean=false){
    Globals.global_lock.synchronized{
      ...
      if(!fromAuto && KarutaPlayUtils.elapsedEnoghSinceLastConfirm(cur_time,KarutaPlayUtils.last_confirmed_for_volume)
      ...
      }else if(!fromAuto && KarutaPlayUtils.elapsedEnoghSinceLastConfirm(cur_time,KarutaPlayUtils.last_confirmed_for_ringer_mode) &&
